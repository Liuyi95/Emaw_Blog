...
import Link from 'next/link'
    export const RichText = {
        ...
        block:{
            ...
            h2: ({ children }: any) => (
                <h2 className="text-3x1 py-10 font-bold"> {children}</h2>
                ),
            h3: ({ children }: any) => (
                <h3 className="text-2xl py-10 font-bold">{children}</h3>
                ),
            h4: ({ children }: any) => (
                <h4 className="text-2x1 py-10 font-bold">{children}</h4>
                ),
            blockquote: ({ children }: any) => (
                <blockquote className="Iborder-l-blue-600 border-1-4 p1-5
                py-5 my-5">
                {children}
                </blockquote>
            ),
